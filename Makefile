...
help:
	...
	@printf "  $(GREEN)install-gpu$(RESET)          Non-interactive install in GPU (FULL) mode\n"
	@printf "  $(GREEN)install-cpu$(RESET)          Non-interactive install in CPU (LITE) mode\n"
	...
	@printf "  $(GREEN)up-gpu$(RESET)               Start stack forcing GPU (FULL) mode\n"
	@printf "  $(GREEN)up-lite$(RESET)              Start stack forcing CPU (LITE) mode\n"
	@printf "  $(GREEN)down$(RESET)                 Stop all services\n"
	@printf "  $(GREEN)build$(RESET)                Build all images without starting\n"
	@printf "  $(GREEN)restart$(RESET)              Quick restart services\n"
	@printf "  $(GREEN)rebuild-rag$(RESET)          Rebuild only the RAG Backend\n"
	@printf "  $(GREEN)reload-nginx$(RESET)         Test and reload Nginx config\n"
	@echo ""
	...
	@printf "  $(GREEN)logs-rag$(RESET)             Logs of RAG Backend only\n"
	@printf "  $(GREEN)logs-init$(RESET)            Logs of ollama_init bootstrap\n"
	@printf "  $(GREEN)logs-ollama$(RESET)          Logs of Ollama service\n"
	@printf "  $(GREEN)logs-qdrant$(RESET)          Logs of Qdrant vector DB\n"
	@printf "  $(GREEN)logs-redis$(RESET)           Logs of Redis cache\n"
	@printf "  $(GREEN)logs-nginx$(RESET)           Logs of Nginx reverse proxy\n"
	@printf "  $(GREEN)logs-webui$(RESET)           Logs of Open WebUI\n"
	@printf "  $(GREEN)logs-console$(RESET)         Logs of Document Console\n"
	@printf "  $(GREEN)monitor$(RESET)              Real-time CPU/RAM resource usage\n"
	@printf "  $(GREEN)monitor-once$(RESET)         One-shot resource snapshot\n"
	@printf "  $(GREEN)gpu-monitor$(RESET)          Real-time GPU usage (nvidia-smi)\n"
	@echo ""
	...
	@printf "  $(GREEN)list-docs$(RESET)            List ingested documents\n"
	@printf "  $(GREEN)init-collection$(RESET)      Init Qdrant collection for current domain\n"
	@printf "  $(GREEN)list-rag-models$(RESET)      List models known by RAG backend\n"
	@printf "  $(GREEN)wipe-rag$(RESET)             Wipe RAG data (Qdrant + SQLite + cache)\n"
	@printf "  $(GREEN)edit-system-prompt$(RESET)   Edit live LLM system prompt\n"
	@echo ""
	@echo "$(CYAN)CONSOLE:$(RESET)"
	@printf "  $(GREEN)up-console$(RESET)           Start console only\n"
	...
	@echo ""
	@echo "$(CYAN)OLLAMA MODELS:$(RESET)"
	@printf "  $(GREEN)list-models$(RESET)          List models installed on Ollama\n"
	@printf "  $(GREEN)active-model$(RESET)         Show model currently loaded in memory\n"
	@printf "  $(GREEN)pull-models$(RESET)          Download all models configured in .env\n"
	@printf "  $(GREEN)pull-model$(RESET)           Pull a specific model (usage: MODEL=mistral:7b)\n"
	@printf "  $(GREEN)remove-model$(RESET)         Remove a specific model (usage: MODEL=gemma2:2b)\n"
	@echo ""
	@echo "$(CYAN)CONFIG & BRANDING:$(RESET)"
	@printf "  $(GREEN)client-info$(RESET)          Show current client/branding config\n"
	@printf "  $(GREEN)reconfigure-client$(RESET)   Update branding without full reinstall\n"
	@echo ""
	@echo "$(CYAN)MAINTENANCE:$(RESET)"
	@printf "  $(GREEN)setup$(RESET)                Initial setup tasks\n"
	@printf "  $(GREEN)backup$(RESET)               Backup volumes and config\n"
	@printf "  $(GREEN)clean$(RESET)                Remove generated artifacts\n"
	@echo ""
